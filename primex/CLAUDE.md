...
# Unit tests (Vitest, node env — pure logic in src/**/*.test.ts + loadtest/**/*.test.mjs)
...
cd ai_worker && pytest             # Unit tests (behavior_tracker, cooldown, heartbeat)
```

Unit tests are Vitest (node env), colocated as `src/**/*.test.ts` — pure logic only (`vitest.config.ts` aliases `@/` and stubs `server-only`). The `include` also picks up `loadtest/**/*.test.mjs`, so the capacity harness's stop rules run with the rest of the suite. The AI worker has unit tests in `ai_worker/tests/`.
...
- **The suite is local-only by construction.** `webServer` runs on port `3100` with `reuseExistingServer: false` and an explicit env from `e2e/env.ts` — never reuse a hand-started dev server. A dev server started by hand inherits `.env.local`, which points at the *remote* Supabase project and carries live Resend/Stripe/DO/Ant Media credentials; the suite resets the database and exercises create/delete flows. `e2eServerEnv()` points at local Supabase and blanks every outward integration so each degrades to its logged no-op.
- Timeouts are deliberately large (`timeout: 300000`, `expect: 15000`): the suite drives `next dev`, so the first visit to each route pays compilation.
...
- **`BehaviorTracker`** (`behavior_tracker.py`) turns raw detections into the detection-type events: `motion_afterhours`, `person_lingering`, `concealment_behavior`, `vehicle_detection`. **There is no `door_event`** — the state dict and its `door_open_threshold_s` knob were removed (SEC-166) because YOLO's COCO classes contain no door, so the path could never fire; don't re-add the knob without a detector that can see a door.
...
- **`HeartbeatState`** (`heartbeat.py`) decides *when* to report frame liveness — it never sends. Successes are throttled to one write per `interval_s` (30 frames/min per camera would otherwise be 30 updates/min saying the same thing); entering or leaving degradation is reported immediately. Sending goes through `EventPoster` to the `camera-heartbeat` edge function, which writes `cameras.last_frame_at` and **nothing else** (SEC-204). That column previously meant "AMS said something", not "we saw video". The edge function deliberately does not touch `cameras.status`: it already has two writers (the AMS webhook and the reconcile cron) and a third with its own notion of "up" would let them fight, with the last writer winning. Liveness is reported as evidence; interpreting it stays with the reconciler.
- Worker config (`confidence_threshold`, `snapshot_interval_s`, `cooldown_s`, `dwell_threshold_s`) is a singleton row in `ai_worker_config` (`config.py`), not env vars. The Supervisor re-reads it on every 30s sync (`_refresh_config()`), so Settings-page edits are picked up by a running worker (SEC-169). Ranges and copy for the UI live in `src/lib/ai-worker-config.ts`.
- **Env vars are named differently from the Next app for the same underlying resources** — don't assume they're shared: `ai_worker/.env` uses `SUPABASE_URL`/`SUPABASE_SERVICE_KEY` (Next uses `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`) and `AI_WORKER_SECRET` (checked by both edge functions — `supabase/functions/ai-event-ingest` and `camera-heartbeat`, the worker's only two write paths into the DB, deliberately so its blast radius stays those endpoints rather than table access). It also writes frames to `DO_SPACES_BUCKET` — a separate bucket from the Next app's `DO_SPACES_RECORDINGS_BUCKET`.
...
> ⚠️ **The ingest architecture is changing: pull is dead, push is the design (decided 2026-09-15).**
> Video now originates *inside* the customer network and is pushed **outbound** over SRT to
> AMS; AMS no longer reaches into a customer LAN. Read **root `docs/streaming-architecture.md`
> before changing anything in this section** — it is the authority, and the code below has not
> caught up (`createStreamSource()` still exists, and there is no SRT in `src/` at all).
> Everything about *playback* is unchanged: it is keyed on `streamId` either way.
...
- **Two ingest modes** (`lib/data/actions/streaming.ts`, both super_admin-only, keyed on `stream_id` so playback/webhooks are identical):
  ...
  - **RTSP pull — ⚠️ LEGACY, on the way out. Do not build on it or extend it.** `createStreamSource()` creates a `type: 'streamSource'` broadcast with `streamUrl` set to the camera's RTSP URL; Ant Media connects *out* and republishes it. This is the architecture that failed in the field: it needs a stable inbound path into the customer LAN, which no consumer ISP guarantees (two hardware generations, 20–100% and 55% measured packet loss — see `docs/streaming-architecture.md`). Under push, the RTSP URL becomes *site-side* config that the on-site pusher reads and **is never sent to AMS**; it stays in `cameras.source_url` (migration 017, distinct from `stream_url`) for provisioning and support only. Still wired into `add-camera-modal.tsx` and `edit-camera-modal.tsx` until the push path replaces it
  - **SRT push (target design, NOT YET IMPLEMENTED)** — the on-site pusher publishes outbound into a `liveStream` broadcast. `createBroadcast()` above is the half that already exists; what is missing is SRT itself, the per-site AES passphrase, and the pusher config bundle

### Streaming Capacity Harness (`loadtest/`, SEC-192)

Every `CameraPlayer` opens a peer connection straight to the single origin Ant Media droplet — there is no edge tier — so the viewer ceiling had to be measured rather than guessed.

```bash
# from primex/ — puts synthetic viewers on a LIVE server
node loadtest/run.mjs --confirm=i-am-load-testing-a-live-server \
  --start=5 --step=5 --max=40 --hold=20000
```

- It refuses to run without the `--confirm` phrase. There is no dry run: a number that didn't come from a loaded server isn't a measurement.
- It creates its own disposable broadcast (`primex-loadtest-<epoch>`), publishes an ffmpeg test pattern into it, and deletes it in a `finally`. **Real camera streams are never the target.**
- Stop rules live in `ramp.mjs` and are unit-tested by `npm test` — deciding when to stop hammering production is logic, not judgement. The first rule checked is "a camera that was already broadcasting went down": anything live at start is real, so the run aborts whether or not it caused the drop.
- Measured 2026-08-11: ≥30 concurrent WebRTC viewers on one stream, ceiling not reached. Write-up in root `docs/streaming-capacity.md`. The result feeds `ANTMEDIA_WEBRTC_VIEWER_LIMIT` — a cap adds no capacity, it only decides that the *next* viewer is refused instead of every stream degrading together.
- **The publish flap that ran alongside this is explained and closed** (2026-09-15): the "RTMP publish drops every 90–180s" was the degraded VPN tunnel on the abandoned pull path, not AMS and not the DVR. The viewer measurement is unaffected and still stands. Don't re-open it as a server fault — see `docs/streaming-architecture.md`.
...
- `ANTMEDIA_WEBRTC_VIEWER_LIMIT`, `ANTMEDIA_HLS_VIEWER_LIMIT` — optional per-broadcast viewer caps applied at provisioning time (`lib/data/actions/streaming.ts`). Unset leaves AMS's unlimited default, and **existing broadcasts keep whatever they were created with** — changing these means re-provisioning the camera. See the capacity harness above before picking a number
- `ANTMEDIA_API_KEY` — the EE **HS256 signing secret** (AMS `jwtSecretKey`), *not* a token. `signRestJwt()` in `lib/streaming/rest-jwt.ts` mints a fresh 60-second JWT per request (header `{alg:HS256,typ:JWT}`, payload `{exp}` only) and sends the raw compact token in `Authorization` with **no `Bearer ` prefix`**. Sending the secret verbatim returns 403 "Invalid App JWT Token". Required in prod: it also switches `getStreamToken()` from tokenless URLs to real per-stream play tokens. Unset on Community Edition. The Python worker signs identically (`ai_worker/antmedia_jwt.py::sign_rest_jwt`) and reads the **same** `ANTMEDIA_API_KEY` — the shared golden fixture keeps the two in step. ⚠️ **That sharing matters for rotation** (overdue — this secret has been exposed in transcripts and shared externally): rotating it in Vercel alone silently breaks the worker's snapshot fetches with a 403 that looks exactly like the IP-allowlist problem. Rotate both sides in one window. Note: prod REST may additionally be gated by IP allowlist — see the Ant Media REST IP allowlist memory
- `DO_SPACES_RECORDINGS_BUCKET`, `DO_SPACES_ENDPOINT` — ⚠️ **There is no recordings bucket on this account.** `primex-recordings` — the name in this repo's specs and `.env.example` — is unprovisioned and exists in no DO region (probed across nine, 2026-08-10). 🚨 **The bucket named `primex` in `sgp1` belongs to a THIRD PARTY — never point recordings at it**, whatever older notes imply. When recordings actually ship, create `primex-recordings` in `nyc3` (co-located with the AMS droplet). Until then this stays unset and recordings stay dark; migration 020's evidentiary-hold logic is applied but no recordings exist (SEC-194)
...
- `ANTMEDIA_SOURCE_ALLOWED_CIDRS` — optional, and **belongs to the legacy pull path**. Narrows which private addresses an RTSP pull source may resolve to (SEC-193). Loopback/link-local/multicast are refused unconditionally; private ranges are allowed by default because tunnelled site cameras genuinely lived there. Under push, AMS never resolves a customer address at all, so this var goes away with `createStreamSource()`
...
The git root is above `primex/` (see the root `CLAUDE.md` for what lives there, including two directory traps). Inside `primex/`:
- `src/` — the Next.js app; `@/*` → `./src/*`
- `supabase/` — **the real** migrations, `seed.sql` and edge functions (the git-root `supabase/` is CLI scratch state only); run `supabase` CLI commands from here
- `ai_worker/` — Python detection worker, deployed separately via Docker (excluded from tsconfig)
- `e2e/` — Playwright suite, local-only by construction
- `loadtest/` — streaming capacity harness (SEC-192)
- `fixtures/` — cross-language golden fixtures, currently the AMS REST JWT pair asserted by both the TS and Python suites

### Deploy

Pushes to `master` hit `.github/workflows/deploy.yml`, whose only step curls the `VERCEL_DEPLOY_HOOK` secret — there is no build, lint, typecheck or test gate in CI. Anything you want enforced before deploy has to be run locally.
